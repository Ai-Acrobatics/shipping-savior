...
export default function ExecutiveSummaryCard({ data }: { data: ExecutiveSummary }) {
  ...
  return (
    <div className="bg-white border border-navy-100 shadow-soft rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-bold text-navy-900">Executive Summary</h2>
          <p className="text-xs text-navy-400 mt-0.5">{data.period}</p>
        </div>
        <span className="text-xs px-3 py-1.5 rounded-full bg-ocean-50 text-ocean-600 font-medium">
          Stakeholder View
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-navy-50 rounded-xl p-4">
          ...
          <div className="text-xl font-bold text-navy-900">{formatCurrency(data.revenue.current)}</div>
          <div className="flex items-center gap-1 mt-1">
            <TrendingUp className="w-3 h-3 text-emerald-600" />
            <span className="text-xs text-emerald-600">+{revenueChange.toFixed(1)}%</span>
        ...
        <div className="bg-navy-50 rounded-xl p-4">
          ...
          <div className="text-xl font-bold text-navy-900">{data.volume.containers} containers</div>
        ...
        <div className="bg-navy-50 rounded-xl p-4">
          ...
          <div className="text-xl font-bold text-navy-900">{formatCurrency(data.savings.total)}</div>
        ...
        <div className="bg-navy-50 rounded-xl p-4">
          ...
          <div className="text-xl font-bold text-navy-900">{data.onTime.rate}%</div>
          <div className="flex items-center gap-1 mt-1">
            {data.onTime.rate >= data.onTime.target ? (
              ...
              <>
                ...
                <span className="text-xs text-red-600">{(data.onTime.target - data.onTime.rate).toFixed(1)}% below target</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs text-navy-500">{formatCurrency(data.revenue.current)} / {formatCurrency(data.revenue.target)}</span>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-red-500/5 border border-red-500/10 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-red-600">Top Risks</span>
          </div>
          <div className="space-y-2">
            {data.topRisks.map((risk, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="w-4 h-4 rounded-full bg-red-50 text-red-600 text-[10px] flex items-center justify-center flex-shrink-0 mt-0.5">
                  {i + 1}
                </span>
        ...
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-emerald-600">Top Wins</span>
          </div>
          <div className="space-y-2">
            {data.topWins.map((win, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="w-4 h-4 rounded-full bg-emerald-50 text-emerald-600 text-[10px] flex items-center justify-center flex-shrink-0 mt-0.5">
                  {i + 1}
                </span>
                <span className="text-xs text-navy-500">{win}</span>
              </div>
            ))}
          </div>
        </div>
